...
function conditionalOptimization(data, q) {
  ...
  for (let k = data.length - 1; k > 0; k--) {
    // Обход каждого шага управляемого процесса, начиная с последнего.
    conditionalMaximums[`Z_${k}`] = {}; // Создание объекта условного масимума на k-м шаге.
    for (const T of data[0]) {
      // Обход массива с данными о времени, подлежащего распределению.
      for (let t = T; t > 0; t -= data[0][0]) {
        // Получение допустимых значений времени.                 TODO: t должен вычитаться слогласно шагу t_1 - t_2
        ...
        if (conditionalMaximums[`Z_${k + 1}`]) {
          // Если существует условный максимум на предыдущем шаге.
